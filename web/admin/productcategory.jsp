...
Ext.onReady(function() {
	...
	var _jsonstore = new
		...
		({
			url: "action/do_productcategory.jsp", // 自定义
			autoDestroy: true,
			autoLoad: {
				params: {
					start: 0
						...
						},
						{
							...
							listeners: {
								click: function(button, e) {
									...
									_upload.preview.set(_form.getForm().findField("pic").getValue());
									_upload_banner.preview.set(_form.getForm().findField("banner").getValue());
								}
							}
						}
/*
...
*/
	...
	function _submit() {
		if (_form.getForm().isValid()) {
			if (!_form.getForm().findField("pic").getValue() || !_form.getForm().findField("banner").getValue()) {
	...
	var _upload = (function() {
		...
		that.preview = {
			...
			src: "http://dummyimage.com/70x80/ffffff/000000.jpg&text=280*320"
		};
			...
			items: [
				// 自定义
				{
					...
					items: [
						...
						{
							html: '<img src="' + that.preview.src + '" alt="" id="' + that.preview.id + '" style="border: 1px solid silver; height: 80px; width: 70px;" />',
							style: {
								"margin-top": "4px"
							}
						}
				...
				that.form.getForm().submit({
					...
					success: function(form, action) {
						...
						if (result.success) {
							...
							// 自定义
							_form.getForm().findField("pic").setValue(pathname);
						}
					},
					failure: function(form, action) {
						var
	...
	var _upload_banner = (function() {
		...
		that.preview = {
			...
			src: "http://dummyimage.com/192x37/ffffff/000000.jpg&text=1920*370"
		};
		...
		that.form = new Ext.form.FormPanel({
			...
			items: [
				// 自定义
				{
					fieldLabel: '<span style="color: red;">*</span> Banner',
					autoWidth: true,
					items: [
						{
							xtype: "fileuploadfield",
						...
						{
							html: '<img src="' + that.preview.src + '" alt="" id="' + that.preview.id + '" style="border: 1px solid silver; height: 37px; width: 192px;" />',
							style: {
								"margin-top": "4px"
							}
						}
				...
				that.form.getForm().submit({
					...
					success: function(form, action) {
						...
						if (result.success) {
							...
							// 自定义
							_form.getForm().findField("banner").setValue(pathname);
						}
					},
					failure: function(form, action) {
						var
